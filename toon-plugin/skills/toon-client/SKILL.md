...
target-chain claim. `pair` is the swap pair (from kind:10032 discovery),
`chainRecipient` is your payout address on `pair.to.chain`, and the swap peer
must be reachable from the selected apex (`btpUrl` names a direct-dialled one).
Returns the decrypted target-chain claim(s) + settlement metadata.

The maker is probed once with a rolling-swap RFQ (kind:20033). A maker that
