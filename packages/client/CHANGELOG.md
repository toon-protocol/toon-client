...
- 883a683: The execution condition leaves the wire (connector ADR 0069, issue #1269).
...
- efd6397: **1.0 — a pure TOON client.**
...
- efd6397: **A metered route is now paid correctly — `send()` never under-pays a `pricePerKib` again.**
  ...
  **A connector URL is now the whole of the configuration.**
  ...
  route a node _forwards_ rather than terminates, since that route is by definition not its own
...
- 1ded4b9: Take up `@toon-protocol/core@3.3.0`'s reseeded genesis peers (and
