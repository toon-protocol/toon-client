...
| `client.sendSwapPacket(…)`        | **swap**  | Multi-chain token swap (low-level). Most callers drive it via the rolling RFQ-then-fills session (`sendRollingRfq` + `handleRollingAdvance`, both exported from this package); the `toon-clientd` daemon behind the `toon_swap` MCP tool orchestrates that session end to end.       |
...
- **[@toon-protocol/sdk](https://www.npmjs.com/package/@toon-protocol/sdk)** — Higher-level helpers for building service nodes (settlement, upload, node bootstrap). A client does **not** swap through the sdk: the rolling-swap session primitives (`sendRollingRfq`, `handleRollingAdvance`) are exported from `@toon-protocol/client` itself
