 ...
 * OnChainChannelClient.test.ts's `fetchMock` pattern), and Mina native sends
...
describe('sendTransfer — dispatch & validation', () => {
  ...
  it('throws InvalidAddressError for a malformed Mina address', async () => {
    await expect(
      sendTransfer(
        { mina: { graphqlUrl: 'http://x', privateKey: 'ab'.repeat(32) } },
        { chain: 'mina', asset: 'native', to: 'not-b62', amount: '1' }
      )
    ).rejects.toThrow(InvalidAddressError);
  });
});
...
describe('sendTransfer — Mina', () => {
  ...
  beforeEach(async () => {
    try {
      ...
      senderPrivateKeyHex = id.mina.privateKey;
      minaAvailable = true;
    } catch {
      minaAvailable = false;
    }
  ...
  it('always throws TransferUnsupportedError for the Mina settlement token', async () => {
    if (!minaAvailable) return; // optional dep absent — skip, do not false-pass
    await expect(
      sendTransfer(
        { mina: { graphqlUrl: 'http://mina', privateKey: senderPrivateKeyHex } },
        { chain: 'mina', asset: 'token', to: RECIPIENT, amount: '1000' }
      )
    ).rejects.toThrow(TransferUnsupportedError);
  });

  function mockMinaFetch(opts: {
    senderBalance: string;
    destBalanceSequence: string[];
  }): typeof fetch {
    let destIdx = 0;
    return vi.fn(async (_url: unknown, init: RequestInit) => {
      ...
      if (body.query.includes('sendPayment')) {
        return new Response(
          JSON.stringify({ data: { sendPayment: { payment: { hash: 'mina-tx-hash' } } } }),
          { status: 200, headers: { 'Content-Type': 'application/json' } }
        );
      }
      // account query — distinguish sender (nonce/inferredNonce) vs destination.
      const isDest = body.variables.pk === RECIPIENT;
      if (isDest) {
        const v = opts.destBalanceSequence[
          Math.min(destIdx, opts.destBalanceSequence.length - 1)
        ];
        destIdx++;
        return new Response(
          JSON.stringify({ data: { account: { balance: { total: v } } } }),
          { status: 200, headers: { 'Content-Type': 'application/json' } }
        );
      }
      return new Response(
        JSON.stringify({
          data: {
            account: {
              balance: { total: opts.senderBalance },
              nonce: '0',
              inferredNonce: '0',
            },
          },
  ...
  it('sends native MINA and confirms by destination balance delta', async () => {
    if (!minaAvailable) return;
    const fetchImpl = mockMinaFetch({
      senderBalance: '5000000000',
      destBalanceSequence: ['0', '1000000'],
    });
    ...
    const result = await sendTransfer(
      ...
      { chain: 'mina', asset: 'native', to: RECIPIENT, amount: '1000000' }
    );

    expect(result.txHash).toBe('mina-tx-hash');
    expect(result.balanceBefore).toBe('0');
  ...
  it('throws TransferNotDeliveredError when accepted but the destination balance never moves', async () => {
    if (!minaAvailable) return;
    const fetchImpl = mockMinaFetch({
      senderBalance: '5000000000',
      destBalanceSequence: ['0', '0', '0', '0'],
    });
    await expect(
      sendTransfer(
        {
