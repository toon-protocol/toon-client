 ...
 *
 * Everything exported here is pure: hidden-service address validation,
 * `socks5h://` URL parsing, and the chain-RPC binding of ADR 0002 — with no Node
 * built-in anywhere behind them. That is what lets config validation — which
 * runs in a browser bundle too — refuse a misconfigured hidden-service connector
 * before a single byte or DNS query leaves the process.
 *
