  ...
  return (
    <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
      <div className="min-w-0 flex-1 space-y-4">
        ...
        {/* Toolbar: branch selector on the left, Code/Push actions on the right */}
        <div className="flex flex-wrap items-center justify-between gap-2">
          {refs ? (
            <BranchSelector
              refs={refs}
              currentRef={currentRef ?? metadata.defaultBranch}
              onSelect={(fullRef) => {
                const short = shortRefName(fullRef);
                navigate(`/${owner}/${repo}/tree/${short}`);
              }}
            />
          ) : (
            <div />
      ...
      <AboutSidebar metadata={metadata} refs={refs} owner={owner} />
    </div>
  );
}
