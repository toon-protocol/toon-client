...
// finished. Any run longer than an hour therefore died at the very last step:
//
...
//
// Fixed upstream in connector#463 (merged 2026-07-26, proven live on a >1h
// run in connector#459). toon-client is among the busiest factory repos —
// one run already hit 42 minutes against the old 50-minute step cap — so it
// carries live headroom risk, not just preventive exposure. Raising
// `timeout-minutes` on its own makes this worse, not better: the extra
// minutes are spent and the push still fails.
//
// THE FIX
// -------
