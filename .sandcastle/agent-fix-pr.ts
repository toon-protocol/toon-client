...
// (toon-meta#357, toon-meta's scripts/factory/{repair,automerge}-evaluator.mjs
// + auto-merge.mjs) applies `agent:fix` to a PR whose ONLY blocker(s) are red
// checks and/or a merge conflict.
//
// Ported from toon-meta's .sandcastle/agent-fix-pr.ts (toon-meta#365 fan-out),
// adapted to toon-client's sandbox conventions (pnpm install hooks — see
// ./agent-review-pr.ts, whose single-PR mechanics this mirrors: fetch the PR
...
// = 2, so this runner does not also need implement's full iteration budget).
//
// THE LOAD-BEARING PART (toon-meta#357's own words): this runner REMOVES its
// own `agent:fix` label when it finishes, WHATEVER THE OUTCOME (fix pushed, no
...
// automated clears: `agent:implement` (toon-meta#330), `needs:human`
// (toon-meta#352), `agent:review` (toon-meta#355). This must not become a
// fourth. Removing the label unconditionally also re-arms the repair
// evaluator's `hasAgentFixInFlight` gate, so the NEXT auto-merge pass
// evaluates this PR fresh (still red → another `repair` attempt against the
// budget, or `escalate` once the budget is spent — never silently stuck
// "in flight" forever).
//
...
// before the try), or a job-level kill (the step timeout, a cancel). Those
// leave `agent:fix` on the PR, and nothing else clears it — the same exposure
// `agent-review-pr.ts` has for `agent:review`.
//
...
// Deliberately a LOCAL constant: .sandcastle/ is what gets fanned out to every
// fleet repo, so this runner must work standalone here without toon-meta-only
// helpers. Mirrors AGENT_REVIEW_LABEL's own local definition in
// review-verdict.ts, one file over.
 ...
 * `review-verdict.ts`'s `agent:review` clear (toon-meta#355) — silently losing
 * this removal would recreate the exact stuck-label problem toon-meta#357
 * exists to fix.
 */
...
const hooks = {
  sandbox: {
    onSandboxReady: [
      ...
      // toon-client is a pnpm monorepo — frozen install, mirroring
      // agent-review-pr.ts (we do NOT copyToWorktree node_modules: pnpm's
