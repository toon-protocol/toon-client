...
Docs-only, no behavior change. `SKILL.md`, `tool-reference.md`, the `toon_swap` MCP tool description, server `instructions`, and `control-api.ts` JSDoc already described the rolling RFQ-then-fills session correctly — those needed no change. Four stragglers did:
...
- `packages/client-mcp/README.md` — the MCP-tool/endpoint table still summarised `toon_swap` as `(destination, amount, {toonData?})` backed by a bare `sendSwapPacket`; now names the tool's real required arguments and the rolling session behind them.
