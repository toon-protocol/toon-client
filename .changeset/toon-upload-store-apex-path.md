...
- **Invalid chainId (F06):** `parseChainId` and the chainId-from-chainKey parsing in `ToonClient`, `client-mcp/config`, and `apex-discovery` now tolerate the 2-part `evm:{chainId}` key some connectors advertise (e.g. `evm:31337`), not only the canonical 3-part `evm:{network}:{chainId}`. A mis-parsed key produced `chainId: 0`, which the store connector rejected.
